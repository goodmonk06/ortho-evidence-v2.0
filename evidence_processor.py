...
class OrthoEvidenceProcessor:
    ...
    def __init__(self, db_path="ortho_evidence.db"):
        ...
        self.conn = None
        self.cursor = None
        self.lock = threading.RLock()  # 追加: スレッドロック
        self.connect_db()
    
    def connect_db(self):
        """データベースに接続"""
        try:
            self.conn = sqlite3.connect(self.db_path, check_same_thread=False)  # 変更: スレッド間で共有可能に
